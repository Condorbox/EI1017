...
class LinearRegressionTest {
    ...
    void testEmptyFileTrain(){
        assertThrows(IllegalArgumentException.class, () -> linearRegression.train(CSVReader.readTable(emptyFile)));
    }

    @Test
    @DisplayName("Test 3 column file")
    void testThreeColumnFileTrain(){
        assertThrows(IllegalArgumentException.class, () -> linearRegression.train(CSVReader.readTable(threeFile)));
    }

    @Test
    @DisplayName("Test miles_dollars train")
    void testMilesDollarsTrain(){
        linearRegression.train(CSVReader.readTable(milesDollars));
        assertAll(
    ...
    void estimateTwoNumbers(){
        linearRegression.train(CSVReader.readTable(numbers));
